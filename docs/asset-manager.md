...
## V3.7.3B Activision Games Blog static allowlist

`scripts/assets/activision-games-blog.mjs` is a controlled fixed integration for three reviewed, current first-party Guitar Hero Live static assets. It is not a general Activision crawler. The registry pairs each exact `blog.activision.com` provenance page with one exact HTTPS `blog.activision.com/content/dam/...` asset URL, known JPEG dimensions and its reviewed role/form: Hero Powers gameplay (1500x500), Rivals Arena gameplay (1920x762), and the Guitar Hero Live controller product (1280x508). It never parses arbitrary article media, crawls, searches, authenticates, uses private APIs, Archive, PlayStation Store, third-party mirrors, or the unavailable `community.activision.com` legacy host as a fallback.

The adapter checks the fixed public provenance page and static asset independently. It validates HTTPS, exact URL pairing, the `blog.activision.com` redirect chain, image MIME type and the registered dimensions before emitting a candidate. The normal path uses `HEAD`. Only when an exact approved `image/jpeg` entry is served as `application/octet-stream` does it issue a bounded `GET Range: bytes=0-4095`, requiring `206`, a valid range starting at byte zero, no body beyond the requested range, and the JPEG `FF D8 FF` signature. This exception never accepts generic MIME globally and fails closed for all other URLs, MIME values, redirects, ranges, or signatures. A missing or invalid static asset fails closed without broadening discovery. The normal generic semantic, visual-utility, score and selection stages decide between the two gameplay candidates; no provider-specific scoring is applied. Cover/clean art, official-art/clean-art and Activision company-logo queries intentionally emit no candidate and remain coverage gaps.

The adapter emits only the existing `copyrighted_editorial` records with `Activision Publishing, Inc.` ownership, complete first-party provenance, `license: null`, `license_status: no_open_license_identified`, and `editorial_use_only: true`. It does not imply permission to reuse Activision material: provenance and attribution are documentation only, and the existing controlled-risk double-consent switches remain mandatory. Reporting preserves the existing activation, fixed-source request, media-preflight, source-failure and catalog-version counters.
...
The report's `editorial_trial` section records whether the adapter was actually registered for applicable coverage, its attempted article requests, media preflights, source failures, safe source-failure details, and canonical catalog version before/after the run. A detail includes provider, URL/hostname, method, stage, HTTP status when present, reason, and observed content type; it never includes response bodies, media bytes, credentials, cookies, or tokens. These are metadata-only counters; no source-media bytes or temporary configuration content enter artifacts.
